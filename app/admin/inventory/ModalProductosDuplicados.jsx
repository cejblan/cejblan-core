          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-yellow-100 text-yellow-800">
                ...
                <th className="p-2 text-left">Nombre (actual / nuevo)</th>
                <th className="p-2 text-left">Cantidad</th>
                <th className="p-2 text-left">Precio</th>
                <th className="p-2 text-left">Precio Mayorista</th>
                <th className="p-2 text-center">¿Reemplazar?</th>
              </tr>
            </thead>
            <tbody>
              {conflictos.map((c, idx) => {
                ...
                return (
                  <tr key={idx} className="border-t">
                    <td className="p-2">{c.nuevo.id}</td>
                    <td className="p-2">
                      <div>
                    ...
                    <td className="p-2">{c.original.quantity} → <b>{c.nuevo.quantity}</b></td>
                    <td className="p-2">{c.original.price} → <b>{c.nuevo.price}</b></td>
                    <td className="p-2">{c.original.wholesale_price} → <b>{c.nuevo.wholesale_price}</b></td>
