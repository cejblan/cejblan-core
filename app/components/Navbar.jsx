...
export default function Navbar() {
  ...
  return (
    <DoNotShowAdmin>
      <nav id="navCejblan" className="bg-gradient-to-b from-[#0A192F] to-[#64FFDA] text-[#F8F8F8] text-xl font-bold justify-center items-center grid max-[420px]:grid-cols-6 grid-cols-12 gap-1 py-1 px-2 shadow-8xl h-fit w-full z-30 fixed">
        {session?.user ?
          <>
        ...
        <Link href="/" className="max-[420px]:col-start-2 max-[420px]:col-end-6 col-start-5 col-end-9 z-10">
          <div className="flex">
            <Image className="m-auto max-[420px]:w-full w-2/3 h-2/3" src={Logo} alt="Logo Cejblan" width={200} height={200} />
          </div>
        </Link>
        {session?.user ? (
