...
export default function Navbar() {
  ...
  useEffect(() => {
    if (session?.user?.paleta_colores) {
      try {
        const parsed = JSON.parse(session.user.paleta_colores);
  ...
  function urlStyle(href) {
    if (pathname === href) {
      return { backgroundColor: colores[2] };
    } else {
  ...
  return (
    <DoNotShowAdmin>
      <nav
        id="navAdmin"
        style={{
          backgroundImage: `linear-gradient(to bottom, ${colores[2]}, ${colores[0]})`,
          color: colores[4],
        }}
      ...
      >
        {session?.user ? (
          <>
            <div className="burger max-[420px]:m-auto col-start-1 col-end-1 justify-center items-center" onClick={handleClick}>
              ...
              <div className={`menu bg-slate-500 text-2xl grid grid-cols-1 gap-1 gap-white absolute left-5 top-5 p-1 rounded-xl z-20 ${isOpen ? "" : "hidden"}`}>
                <Link href="/" className={`hover:text-[${colores[0]}]`}>
                  <p
                    style={urlStyle("/")}
                ...
                <Link href="/products" className={`hover:text-[${colores[0]}]`}>
                  <p
                ...
                <Link href="/wishlist" className={`hover:text-[${colores[0]}] max-[420px]:block hidden`}>
                  <p
                ...
                <Link href="/cart" className={`hover:text-[${colores[0]}] max-[420px]:block hidden`}>
                  <p
                    style={urlStyle("/cart")}
                    className={`rounded-xl px-2 flex justify-center items-center`}>
                    Carrito
                ...
                <Link href="/orders" className={`hover:text-[${colores[0]}]`}>
                  <p
                ...
                <Link href="/profile" className={`hover:text-[${colores[0]}]`}>
                  <p
                ...
                <button
                  className={`hover:text-[${colores[0]}] bg-slate-700 rounded-xl px-2 flex justify-center items-center`}
            ...
            <Link href="/wishlist" className={`hover:text-[${colores[0]}] text-sm max-[420px]:hidden col-start-2 col-end-3 block`}>
              <p className={`${urlClass2("/wishlist")} hover:border-[${colores[0]}] rounded-xl`}>
                Favoritos
                <IoIosHeart className="m-auto w-4 h-4" />
              </p>
            </Link>
            <Link href="/cart" className={`hover:text-[${colores[0]}] text-sm max-[420px]:hidden col-start-3 col-end-4 block`}>
              <p className={`${urlClass2("/cart")} hover:border-[${colores[0]}] rounded-xl w-4/5`}>
                Carrito
          ...
          <button
            className={`hover:text-[${colores[0]}] max-[420px]:text-sm text-2xl col-start-1 max-[420px]:col-end-2 col-end-3 flex justify-center items-center`}
            onClick={() => signIn()}
          >
            <p className={`hover:border-[${colores[0]}] max-[420px]:leading-3 leading-6 rounded-xl`}>Iniciar Sesión</p>
        ...
        {session?.user ? (
          ...
          <div className="max-[420px]:m-auto max-[420px]:col-start-6 max-[420px]:col-end-6 col-start-10 col-end-13 flex max-[420px]:justify-center justify-end items-center">
            {onClick && <SearchBar />}
            <button onClick={Open}>
              <PiMagnifyingGlassBold className={`w-4 h-4 hover:fill-[${colores[0]}]`} />
            </button>
          </div>
        )}
      </nav>
